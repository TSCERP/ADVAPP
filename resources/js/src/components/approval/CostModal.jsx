...
const titleMapping = {
    view: {
        cost: "View Purchasing Item",
        additionalCost: "View Additional Purchasing Item",
    },
    create: {
        cost: "Create Purchasing Item",
        additionalCost: "Create Additional Purchasing Item",
    },
    edit: {
        cost: "Edit Purchasing Item",
        additionalCost: "Edit Additional Purchasing Item",
    },
    duplicate: {
        cost: "Create Purchasing Item",
        additionalCost: "Create Additional Purchasing Item",
...
const CostModal = (props) => {
    ...
    // States
    const [vendorList, setVendorList] = useState(tempVendors || []);
    const [vendorSites, setVendorSites] = useState(tempVendorSites || []);
    ...
    // Đối với mode view, duplicate và edit nhớ thêm giá trị mặc định vào !!!
    ...
    const [exchangeRate, setExchangeRate] = useState(info?.exchangeRate || 1);
    ...
    const [selectedVAT, setSelectedVat] = useState(info?.vat || 0);
    ...
    useEffect(() => {
        if (
            ...
            if (selectedVAT) {
                let currentVATValue = costVAT.find(($) => $.id == selectedVAT)
                    ? costVAT.find(($) => $.id == selectedVAT).amount
                    : 0;
    ...
    if (
        ...
        !["cost", "additionalCost"].includes(type)
    )
        return <></>;   
